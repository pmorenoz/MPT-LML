...
# Command:
# Fig. 1 Left: python exp_2_area_fig1.py -n 1000 -d 200 -k 5 -p 1
# Fig. 1 Middle: python exp_2_area_fig1.py -n 1000 -d 200 -k 5 -p 100
...
for i, masked_loss_m in enumerate(list_of_areas):
    ...
    ax.plot(torch.arange(D), masked_loss_m, lw=2.0, color=palette_blue[i], alpha=0.8)

ax.set_xlim(0.0, D-1)
ax.set_xticks([2,30,60,100,140,170,199])
ax.set_xticklabels([r'1{\%}', r'15{\%}', r'30{\%}', r'50{\%}', r'70{\%}', r'85{\%}', r'100{\%}'])
plt.title(r'{\#} \textsc{tokens} $= 200$  / Max. permutations $= '+str(args.max_perm)+'$')
plt.ylabel(r'\textsc{mpt} loss')
plt.xlabel(r'{\%} of masked tokens')
plt.savefig('./plots/area_fig1.pdf')
plt.show()
