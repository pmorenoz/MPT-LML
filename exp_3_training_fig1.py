...
# Command:
# python exp_3_training_fig1.py -e 1000 -d 10 -n 2000
...
for i, _ in enumerate(list_of_lml):
    if i == 0:
        loss_line, = ax.plot(list_of_loss[i], lw=2.0, alpha=0.2, color=palette_pink[i])
        lml_line, = ax.plot(list_of_lml[i], lw=2.0, alpha=1.0, color=palette_pink[i])
    else:
        ax.plot(list_of_loss[i], lw=2.0, alpha=0.2, color=palette_pink[i])
...
plt.savefig('./plots/exp3_fig1.pdf')
plt.show()
