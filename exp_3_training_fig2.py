...
for i, _ in enumerate(list_of_lml):
    if i == 0:
        loss_line, = ax.plot(list_of_loss[i], lw=2.0, alpha=0.2, color=palette_super_red[i])
        lml_line, = ax.plot(list_of_lml[i], lw=2.0, alpha=1.0, color=palette_super_red[i])
    else:
        ax.plot(list_of_loss[i], lw=2.0, alpha=0.2, color=palette_super_red[i])
        ax.plot(list_of_lml[i], lw=2.0, alpha=1.0, color=palette_super_red[i])
...
plt.savefig('./plots/exp3_fig2.pdf')
plt.show()
...
plt.xlabel(r'Epochs')
plt.title(r'Training curves / {\#} \textsc{tokens} = 10 / {\#} \textsc{masks} = 1')
plt.savefig('./plots/exp3_fig2zoom.pdf')
plt.show()
